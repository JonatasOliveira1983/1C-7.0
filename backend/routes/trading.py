...
@router.post("/bypass-nuke-paper")
async def bypass_nuke_paper_state():
    _, bybit_rest_service, _, _, bankroll_manager = get_services()
    from services.firebase_service import firebase_service
    ...
    if bybit_rest_service:
        ...
        try:
            ...
            cleared.append(f"disk-sync-error: {str(e)}")
            
    if firebase_service:
        for i in range(1, 5):
            try: 
                await firebase_service.hard_reset_slot(i, "BYPASS_NUKE_API", pnl=0.0)
            except: 
                pass
        try:
            await firebase_service.update_bankroll(100.0)
        ...
        async with database_service.AsyncSessionLocal() as session:
            ...
            for i in range(1, 5):
                await session.execute(text("""
                    UPDATE slots SET
                        symbol = NULL, side = NULL, qty = 0.0, entry_price = 0.0, entry_margin = 0.0,
                        current_stop = 0.0, initial_stop = 0.0, order_id = NULL, target_price = 0.0,
                        leverage = 50.0, slot_type = NULL, status_risco = 'LIVRE', pnl_percent = 0.0,
                        strategy = NULL, genesis_id = NULL, opened_at = NULL,
                        pensamento = 'Resetado via Bypass Nuke HTTP API', liq_price = 0.0, score = 0.0
                    WHERE id = :slot_id
                """), {"slot_id": i})
            await session.commit()
            cleared.append("postgres_production_slots_and_banca")
    except Exception as pg_err:
        cleared.append(f"postgres_error: {str(pg_err)}")
